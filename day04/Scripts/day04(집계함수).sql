...
SELECT COUNT(COMMISSION_PCT),COUNT(NVL(COMMISSION_PCT, 0))-- NULL값 제거 함수 사용 비교
FROM EMPLOYEES;
...
SELECT DEPARTMENT_ID, AVG(SALARY)
FROM EMPLOYEES 
WHERE SALARY >= 5000 --얘는 전체에 조건을 걸어주는 것/WHERE절에는 집계함수 불가
GROUP BY DEPARTMENT_ID ;
...
SELECT ts.STU_DEPT 학과, ts.STU_NAME 이름, ts.STU_AGE 나이, ts.STU_PHONE 연락처, S.학생수
FROM TBL_STU ts 
JOIN(
	SELECT STU_DEPT 학과, COUNT(*) 학생수 
	FROM TBL_STU ts2 
