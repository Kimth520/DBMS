...
--컬럼 타입 수정
--ALTER TABLE 테이블명 MODIFIY(컬럼명 자료형(용량));
...
CREATE TABLE TBL_ANIMAL(
	ANIMAL_NUMBER NUMBER,--CONSTRAINT PK_ANINMAL PRIMARY KEY
...
--PK는 모든 테이블에 하나만 존재하므로 테이블명으로 하는게 좋음
...
--RENAME CONSTRAINT 는 오라클레서 지원하지 않음

--ANIMAL 테이블에 데이터 추가 -> DML INSERT
--DML 명령어 : SELECT UPDATE INSERT DELETE
--DDL 명령어 : CREAT ALTER DROP TRUNCATE

INSERT INTO TBL_ANIMAL
VALUES(1, 1, 5, 'F');

--INSERT INTO TBL_ANIMAL ta 
...
--STU_GENDER CHAR(1) DEFAULT 'M' NOT NULL CHECK(STU_GENDER = 'M' OR STU-GENDER = 'W'),
--DEFAULT 'M' : 데이터가 들어오지 않으면 무조건 'M' 이 기본값으로 들어가게 설정한다
--NOT NULL : 디폴트값을 M로 설정했으니 NULL값을 강제로 넣는 것을 막기위해 사용하는 제약조건
--CHECK(STU_GENDER = 'M' OR STU-GENDER = 'W') : 이 컬럼의 값은 M또는 W만 들어오도록 하는 제약 조건
...
MEMBER_AGE NUMBER,
MEMBER_PHONENUMBER VARCHAR2(15),
MEMBER_ADDRESS VARCHAR2(100),
CONSTRAINT PK_MEMBER PRIMARY KEY(MEMBER_NUMBER)
);

INSERT INTO TBL_MEMBER
VALUES(1, '김태현', 26, '01045','원주');

SELECT * FROM TBL_MEMBER;
DROP TABLE TBL_MEMBER ; --FOREIGN 키가 묶여있기 때문에 단독으로 삭제가 안됨

CREATE TABLE TBL_BOOK(
BOOK_NUMBER NUMBER,
BOOK_NAME VARCHAR2(100),
BOOK_GENRE VARCHAR2(100) DEFAULT 'IT' NOT NULL CONSTRAINT CHECK_GE CHECK(BOOK_GENRE = '인문학' OR BOOK_GENRE = 'IT' OR BOOK_GENRE = '추리'),
MEMBER_NUMBER NUMBER,
