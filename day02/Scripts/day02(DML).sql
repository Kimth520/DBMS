...
--학생 테이블 생성
--CREAT TABLE 테이블명(
-- 컬럼명 데이터타입 - 문자(VARCHAR2,CHAR), 숫자(MEMBER), 날짜(DATE)
--);
...
SELECT STUDENT_NAME 이름, NVL2(STUDENT_KOR, '점수있음', '점수없음') 국어점수 --0, '점수 없음' 안되는 이유?
FROM TBL_STUDENT;
...
UPDATE TBL_STUDENT 
SET STUDENT_KOR = NVL(STUDENT_KOR, 0);--수정도 가능
...
FROM TBL_STUDENT 
WHERE (STUDENT_MATH >=5 AND STUDENT_ENG>=50 AND STUDENT_KOR>=50)AND (STUDENT_MATH + STUDENT_ENG + STUDENT_KOR)/3>=80;

-- Q2. 학생의 수학, 영어, 국어 점수 중 한 과목이라도 30점 미만이면 테이블에서 삭제하기
DELETE FROM TBL_STUDENT 
WHERE (STUDENT_MATH + STUDENT_ENG + STUDENT_KOR)/3<30;
